  ...
  \verbatim
    <model:physical name="test_model">
      <body:empty name="body_name">
          <controller:gazebo_ros_joint_force name="ros_ray_sensor_controller" plugin="libgazebo_ros_joint_force.so">
            <alwaysOn>true</alwaysOn>
            <updateRate>15.0</updateRate>
            <topicName>jointfoce_topic_name</topicName>
            <frameName>test_model</frameName>
            <interface:opaque name="jointforce_iface" />
          </controller:gazebo_ros_joint_force>
      </body:empty>
    </model:phyiscal>
  \endverbatim
...
class GazeboRosJointForce : public Controller
{
  /// \brief Constructor
    public: GazeboRosJointForce(Entity *parent );

  /// \brief Destructor
    public: virtual ~GazeboRosJointForce();

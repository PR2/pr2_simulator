...
#include <pr2_gazebo_plugins/gazebo_ros_laser.h>
    ...
    : Controller(parent)
{
  ...
  this->topicNameP = new ParamT<std::string>("topicName", "default_gazebo_ros_laser_topic", 0);
...
void GazeboRosLaser::LoadChild(XMLConfigNode *node)
{
  ...
  ROS_WARN("WARNING: gazebo_ros_laser plugin artifically sets minimum intensity to %f due to cutoff in hokuyo filters." , this->hokuyoMinIntensity);
